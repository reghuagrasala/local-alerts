...
// Loaded after app.js so the location button uses this more tolerant handler.
(function(){
  ...
  function showPosition(p){
    ...
    window.__localAlertsLastPosition={lat,lon,accuracy:acc};
    setGps("GPS On","gps-on");
    loc.textContent=`GPS ${lat.toFixed(6)}, ${lon.toFixed(6)}${accuracyText} · finding address…`;
    if(typeof window.maybeRefreshAreaData==="function")window.maybeRefreshAreaData(lat,lon);
    if(typeof window.startLocationWatchSafe==="function")window.startLocationWatchSafe();
  ...
  function getLocation(){
    ...
    setGps("GPS: Getting…","gps-weak");
    loc.textContent="Getting precise location…";
    navigator.geolocation.getCurrentPosition(showPosition,err=>{
      if(err&&err.code===1){setGps("GPS Off","gps-off");loc.textContent="GPS permission not granted";return;}
      // Safari can take longer than 15–20 seconds to obtain a high-accuracy fix.
      // On timeout, retry without forcing high accuracy so Wi‑Fi/cellular location can respond.
      if(err&&err.code===3){
        setGps("GPS: Retrying…","gps-weak");
        loc.textContent="Precise GPS is taking longer. Trying device location…";
      ...
      setGps("GPS Weak","gps-weak");
      loc.textContent="GPS location unavailable — tap the location button to retry";
